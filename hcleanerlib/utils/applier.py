...
class Applier:
    """Class to apply rules on the different element based on the configuration file"""

    def __init__(self, config_type):
        self.__config = Configuration(config_type)

    def apply_folder_rules(self, element):
        """Apply the rules on folders"""
    ...
    def apply_file_rules(self, element):
        ...
        for rule in self.__config.get_file_rules():
            if len(rule) == 4:
                ...
                exclusion = []
            if (
                rule[2] == "*" or file_extension.lstrip(".") in sets[rule[2]]
